...
var _fixedHeader = function _fixedHeader() {
  ...
  if (window.pageYOffset > sticky) {
    ...
    if (window.innerWidth <= 768) {
      // Umbral para móviles (puedes ajustar el valor)
...
var _videoEdificios = function _videoEdificios() {
  var video = document.getElementById('edificios');
  var videoHasEnded = false; // Variable para rastrear si el video ya terminó
  // Función para reproducir y pausar el video según el scroll

  var playVideoOnScroll = function playVideoOnScroll() {
    var videoPosition = video.getBoundingClientRect().top;
    var windowHeight = window.innerHeight; // Verifica si el video está en la vista

    var isInView = videoPosition < windowHeight && videoPosition > 0; // Solo reproducir si el video no ha terminado y está visible
  ...
  window.addEventListener('resize', playVideoOnScroll); // Verificar la visibilidad del video al cargar la página

  document.addEventListener('DOMContentLoaded', playVideoOnScroll);
};

var _videoTendenze = function _videoTendenze() {
  var video = document.getElementById('publicidad');
  var videoHasEnded = false; // Variable para rastrear si el video ya terminó

  var audioActivated = false; // Variable para evitar activar el audio más de una vez
  // Función para reproducir y pausar el video según el scroll

  var playVideoOnScroll = function playVideoOnScroll() {
    var videoPosition = video.getBoundingClientRect().top;
    var windowHeight = window.innerHeight; // Solo reproducir si el video no ha terminado

    if (videoPosition < windowHeight && videoPosition > 0 && !videoHasEnded) {
      video.play(); // Activar el audio después de 1 segundo si aún no está activado
...
var _videoEdificiosSoport = function _videoEdificiosSoport() {
  var video = document.getElementById('edificios');
  var videoHasPlayed = false; // Variable para rastrear si el video ya se ha reproducido
  ...
  function checkIfInView(video) {
    ...
    if (inView && !videoHasPlayed) {
      var dataSrc = video.dataset.src; // Cargar el video solo si no está cargado
